            ...
            await new Promise(resolve => {
                test(testDescription, async (t) => {
                    ...
                    try {
                        ...
                        timeout = setTimeout(() => timeoutFunc('uexpresso'), 60000)
                        let uExpressOutput = (await exec(`node ${testPath}`, {maxBuffer: 1024 * 1024 * 100})).stdout;
                        clearTimeout(timeout);
                        const normalizeOutput = (s) => s.replaceAll('NullObject <[Object: null prototype] {}> ', '').replaceAll('[Object: null prototype] ', '');
                        assert.strictEqual(normalizeOutput(uExpressOutput), normalizeOutput(expressOutput));
                    } catch (error) {
                        console.log(error)
                        throw error;
                    } finally {
