...
app.get('/url-status', (req, res) => {
    // deprecated: res.redirect(url, status)
...
app.listen(13333, async () => {
    ...
    const res = await fetch('http://localhost:13333/url-status', { redirect: 'manual' });
    console.log('deprecated: res.redirect(url, status):', res.status, res.headers.get('Location'));
