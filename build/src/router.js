...
let resCodes = new NullObject(), resDecMethods = ['set', 'setHeader', 'header', 'send', 'end', 'append', 'status'];
...
module.exports = class Router extends EventEmitter {
                        ...
                        if(optimizedPathToRouter) {
                            ...
                            setTimeout(() => {
                                if(!this.listenCalled) {
                                    return; // can only optimize router whose parent is listening
                                }
    ...
    handleRequest(res, req) {
        ...
        // INVARIANT: request.res and response.req must be assigned before any
        // uWS callback (onData / onAborted) can fire. Request's onData
        // backpressure path reads `this.res?.finished` to decide whether
        // _res.pause()/resume() is safe; if request.res were unset when
        // onData first fires, the guard would silently bypass and could
        // crash with 'HttpResponse must not be accessed after ... a
        // successful response'. uWS schedules these callbacks asynchronously,
        // so the synchronous window here is empty in practice — but do not
        // re-order or defer these assignments without re-validating that
        // assumption.
    ...
    _registerUwsRoute(route, optimizedPath) {
        ...
        if(
            ...
            this.get('declarative responses') // must have declarative response enabled
        ) {
            const decRes = compileDeclarative(route.callbacks[0], this);
            if(decRes) {
                fn = decRes;
                if(!this._declarativeRoutes) this._declarativeRoutes = new Set();
                this._declarativeRoutes.add(route.path);
            }
        } else {
