...
class Application extends Router {
    constructor(settings = new NullObject()) {
        ...
        if(typeof settings.fsWorkers !== 'number') {
            settings.fsWorkers = cpuCount > 1 ? 1 : 0;
        }
        if(settings.uwsApp) {
            ...
            if(!settings.uwsOptions.key_file_name || !settings.uwsOptions.cert_file_name) {
                throw new Error('ssl settings are required for HTTP/3');
            }
        ...
        this.workers = [];
        for(let i = 0; i < settings.fsWorkers; i++) {
            if(workers[i]) {
    ...
    listen(port, host, callback) {
        ...
        const onListen = socket => {
            if(!socket) {
                ...
                let err = new Error(`listen EADDRINUSE: address already in use ${addr}:` + port);
                err.code = 'EADDRINUSE';
